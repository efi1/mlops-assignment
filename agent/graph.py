...
def llm() -> ChatOpenAI:
    ...
    return ChatOpenAI(
        ...
        api_key=LLM_API_KEY,
        temperature=0.0,
        max_completion_tokens=256,    # cap output — bounds worst-case generation time        
    )
...
def _parse_verdict(text: str) -> tuple[bool, str]:
    """Parse the verifier's {"ok": bool, "issue": str} reply defensively."""
    match = re.search(r"\{.*\}", text, re.DOTALL)
    if not match:
        # Couldn't find JSON — treat as ok to avoid spurious revise loops.
        return True, ""
    try:
        obj = json.loads(match.group(0))
        return bool(obj.get("ok", True)), str(obj.get("issue", ""))
